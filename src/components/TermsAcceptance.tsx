...
export default function TermsAcceptance({ isOpen, onAccept, onDecline }: TermsAcceptanceProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onDecline(); }}>
      <DialogContent className="max-w-2xl max-h-[60vh] flex flex-col border-2 border-purple-200 dark:border-purple-800 shadow-2xl overflow-hidden relative bg-background" onInteractOutside={(e) => e.preventDefault()}>
