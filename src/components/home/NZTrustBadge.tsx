      ...
      <div className="container mx-auto px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-center mb-10 md:mb-14">
            <div className="inline-flex items-center gap-4 bg-secondary-foreground/10 rounded-2xl px-6 md:px-8 py-4 mb-6">
              <img src={iconShield} alt="" className="w-14 h-14 md:w-16 md:h-16" />
              <div className="text-left">
                <div className="text-xl md:text-3xl font-bold text-secondary-foreground font-display">100% Local Sitters</div>
                <div className="text-secondary-foreground/60 text-xs md:text-sm font-body">Every sitter is local, vetted, and loves pets</div>
              </div>
            </div>
            <p className="text-secondary-foreground/60 text-sm md:text-lg max-w-2xl mx-auto font-body">
              Every ZiggySitters pet sitter is a vetted local. Real people in your neighbourhood who genuinely love animals.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-4 md:gap-6 mb-10">
            {[
              ...
              { icon: iconShield, title: "ID Vetted", desc: "Government ID checked" },
              { icon: iconCamera, title: "Daily Updates", desc: "Photos & care notes" },
            ].map((item) => (
              <div key={item.title} className="flex items-center gap-4 bg-secondary-foreground/5 rounded-xl p-4 md:p-5 border border-secondary-foreground/10">
                <div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-primary/20 flex items-center justify-center flex-shrink-0">
                  <img src={item.icon} alt="" className="w-9 h-9 md:w-10 md:h-10" />
                </div>
                <div>
